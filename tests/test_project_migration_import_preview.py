...
class TestProjectMigrationImportPreview(TransactionCase):
    ...
    def test_build_preview_parses_expected_sheets_and_writes_report(self):
        with TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                ...
                {"Projects", "Assignments", "Timesheets", "Cashflows", "Unmatched", "Warnings", "Summary", "Employee Matches"},
            )
    ...
    def test_cashflow_parser_checks_month_total_and_reports_unmatched(self):
        with TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue(any(row.kind == "cashflow" and row.key == "Cash-OUT Bank Statement" for row in unmatched))
